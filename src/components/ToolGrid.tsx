...
const ToolGrid: React.FC = () => {
  ...
  return (
    <section id="hub" className="max-w-7xl mx-auto px-6 py-40">
      <div className="flex flex-col gap-16 mb-24">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-12">
          ...
          >
            ...
            <h2 className="text-5xl md:text-7xl font-black font-heading text-white tracking-tighter leading-none">
              Stack de <br />
          ...
          >
            ...
            <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-600 group-focus-within:text-accent transition-colors" size={22} />
            <input 
              type="text" 
              placeholder="Encontrar a próxima frequência..." 
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full h-20 bg-white/5 border border-white/10 rounded-2xl pl-16 pr-8 text-xl text-white placeholder:text-slate-700 outline-none focus:border-accent/40 focus:bg-white/10 transition-all font-sans relative z-10 glass-premium"
            />
          </motion.div>
        </div>
        ...
        >
          {categories.map((cat) => (
            <motion.button
               ...
               whileTap={{ scale: 0.95 }}
               className={`flex items-center gap-3 px-8 py-4 rounded-xl font-black text-xs uppercase tracking-[0.2em] transition-all whitespace-nowrap border ${
                 activeCategory === cat 
      ...
      <div className="space-y-32">
        <AnimatePresence mode="popLayout">
          ...
          {!searchQuery && activeCategory === 'all' && discovery.length > 0 && (
            ...
            >
              <div className="flex items-center gap-6 group">
                <div className="w-14 h-14 rounded-2xl bg-accent/20 border border-accent/40 flex items-center justify-center text-4xl group-hover:bg-accent/30 transition-all duration-700 animate-pulse">
                  🔥
                </div>
                <div className="space-y-1 flex-grow">
                  <div className="flex items-center gap-3">
                    <h3 className="text-3xl font-black font-heading text-white tracking-widest uppercase">
                      Rising Technologies
                    </h3>
          ...
          {filteredData.map((section) => (
            ...
            >
              <div className="flex items-center gap-6 group">
                <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-4xl grayscale group-hover:grayscale-0 group-hover:bg-accent/10 group-hover:border-accent/30 transition-all duration-700 rotate-3 group-hover:rotate-0">
                  {section.icon}
                </div>
                <div className="space-y-1 flex-grow">
                  <h3 className="text-3xl font-black font-heading text-white tracking-widest uppercase">
      ...
      {filteredData.length === 0 && (
        ...
        >
          ...
          <div className="space-y-3">
            <h3 className="text-4xl font-black text-white uppercase tracking-tighter">Frequência não encontrada</h3>
